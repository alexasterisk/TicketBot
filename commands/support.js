...
const { modRoleId, supportCategoryId } = require('../config.json')
...
async function createThread (type, interaction, _, data) {
    ...
    if (amountOfThreads >= data.maximum) return interaction.reply({
        content: 'Sorry, you cannot open any more support tickets for this category!\nYou have reached the maximum amount of general support tickets.',
        ephemeral: true
    })

    interaction.guild.channels.create(`${type}-${amountOfThreadsInType + 1}`, {
        type: 'GUILD_TEXT',
        topic: `Ticket ${amountOfThreadsInType + 1} - ${data.fullName}`,
        ...
        permissionOverwrites: [{
            ...
            id: interaction.guild.roles.cache.get(modRoleId),
        ...
        .catch(error => {
            console.log(error)
            interaction.reply({
                content: 'There was an error opening your ticket, please try again or contact a staff member for help.',
                ephemeral: true
            })
        })
}
...
module.exports = {
    ...
    buttons: {
        general_support: { async execute (interaction, _, data) { await createThread('general_support', interaction, _, data) } },
        user_report: { async execute (interaction, _, data) { await createThread('user_report', interaction, _, data) } },
        bug_report: { async execute (interaction, _, data) { await createThread('bug_report', interaction, _, data) } },
        suggestion: { async execute (interaction, _, data) { await createThread('suggestion', interaction, _, data) } },
    },
